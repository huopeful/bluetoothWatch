<template>
  <div class="app-container">
    ...
    >
      <el-table-column :label="$t('table.id')" prop="id" sortable="custom" align="center" width="80" type='index'>
     ...
     <!--  <el-table-column :label="$t('table.deviceNumber')" width="90px" align="center">
      ...
      >
        <template slot-scope="{row}">
          ...
          <el-button size="small" v-waves type="danger" @click="untying(row)">绑定</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="dialogJBVisible" title="绑定">
      <el-form
        ref="untyingForm"
        :rules="untyingRules"
        :model="untyingData"
      ...
      >
       <el-form-item :label="$t('table.deviceList.deviceName')" prop="deviceName">
          <el-select v-model="untyingData.beaconId" class="filter-item" placeholder="Please select">
            <el-option
...
import {getBindDeviceList,unbindBeacon,bindBeacon,unbindBeaconMessage,deviceList} from "@/api/device";
...
export default {
  ...
  methods: {
    ...
    untying(row){
        ...
        console.log('deviceunbind',response);
        var calendarType=[];
        for(var i=0;i<response.data.length;i++){
          var obj={};
          obj.deviceName=response.data[i].deviceName;
          obj.beaconId=response.data[i].id;
          calendarType.push(obj);
        }
        calendarTypeOptions=calendarType;
        this.calendarTypeOptions=calendarTypeOptions;
        console.log('be',this.calendarTypeOptions);
        this.untyingData.prisonerId=row.id;
    ...
    confirmUntying(){
      this.$refs['untyingForm'].validate((valid) => {
        if (valid) {
          bindBeacon({
       prisonerId:this.untyingData.prisonerId,
       beaconId:this.untyingData.beaconId
      }).then(response => {

<template>
  <div class="app-container">
    ...
    >
      <el-table-column :label="$t('table.id')"  type="index" prop="id" sortable="custom" align="center" width="80">
      ...
      <el-table-column :label="$t('table.date')" width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.exceptionTime | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.address')" min-width="150px"  align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('table.healthWarning.state')" min-width="150px"  align="center">
        <template slot-scope="scope">
          <!-- 1为正在报警  2为报警预停止 -->
          <span v-if="scope.row.opinion == 0" class="red">
            正在报警
          </span>
      ...
      >
        <template slot-scope="{row}">
          ...
          <el-button size="small" v-waves type="danger" @click="untying(row)" v-if='row.opinion == 0'>处理报警</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="dialogJBVisible" title="处理报警">
      ...
      >
        ...
        <el-form-item label-width='80px' :label="$t('table.healthWarning.remarks')" prop="remarks">
          <el-input  type="textarea" :rows="2"   v-model="untyingData.remarks"/>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
import { alarm,handleAlarm } from "@/api/health";
  ...
  methods: {
    ...
    getList() {
      ...
      this.list=[];
        ...
        this.screenHealthAlarmData(response.data);
    ...
    untying(row){
      ...
      console.log(row);
      this.untyingData.criminalId=row.userId;
      this.untyingData.opinion=1;
      this.untyingData.exceptionType=row.exceptionType;
      this.untyingData.handlerId=1;
    ...
    confirmUntying(){
      this.$refs['untyingForm'].validate((valid) => {
        if (valid) {
           handleAlarm(this.untyingData).then(response => {
    ...
    // 从众多报警中筛选出是健康报警的数据
    screenHealthAlarmData(data){
    for(var i=0;i<data.length;i++){
      if(data[i].exceptionType==4||data[i].exceptionType==5||data[i].exceptionType==6||data[i].exceptionType==7){
